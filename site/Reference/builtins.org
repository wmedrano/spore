...
** println
:PROPERTIES:
:CUSTOM_ID: Miscprint-y8vhci4109k0
:END:

~println~ prints out all values (+ a newline character) and returns
nothing.

#+BEGIN_SRC lisp
(print (list 1 2 3) " summed is " (+ 1 2 3))
;; (1 2 3) summed is 6
#+END_SRC
...
In a default REPL, there are 2 modules. The global module ~"%global%"~
and ~"%virtual%/repl"~ for items defined in the REPL.
...
** module-info
:PROPERTIES:
:CUSTOM_ID: Miscmoduleinfo-khb4ab4129k0
:END:

~module-info~ returns a ~string~ that has module details such as the
name, imports, and defined values.

#+BEGIN_SRC lisp
>> (define pi 3.14)
>> (import "mylib.spore")
>> (print (module-info "%virtual%/repl"))
Module: %virtual%/repl
  Import /home/wmedrano/src/spore/mylib.spore as mylib
  'pi => 3.14
#+END_SRC
