...
module can be referenced with the syntax ~<alias>/<symbol>~ where
~alias~ is the name of the file stripped of its file extension and
~<symbol>~ is any item defined in the module.
...
** aliases
:PROPERTIES:
:CUSTOM_ID: Miscaliases-i2u9chk0z8k0
:END:

~aliases~ returns the list of available aliases for the given module.
